//! `essh self` - manage the installed binary itself.
//!
//! `self update` shells out to `cargo install easyssh --force`, the crate name even when the command you type is shorter.
...
fn update(args: UpdateArgs) {
    ...
    println!(
        "{} {}\n",
        "Updating easyssh via".dimmed(),
        "cargo install easyssh --force".bold()
    );
...
fn check() {
    ...
    match latest() {
        Ok(latest) if newer(&latest, current) => {
            println!(
                "{} {} {}",
                format!("easyssh {latest}").bold(),
                "is available, you have".dimmed(),
                current.bold()
            );
            println!("{} {}", "run".dimmed(), "essh self update".bold());
        }
        Ok(_) => println!(
            "{} {}",
            format!("easyssh {current}").bold(),
            "is the latest release.".dimmed()
        ),
        Err(e) => {
            eprintln!("{} {e}", "essh: could not reach crates.io:".red());
            std::process::exit(1);
        }
    }
}

/// `cargo search` prints `easyssh = "X.Y.Z"    # description` for an exact name
...
fn confirm() -> bool {
    print!(
        "{} {} ",
        "Update easyssh to the latest release via cargo?".bold(),
        "[y/N]".dimmed()
    );
    io::stdout().flush().ok();
